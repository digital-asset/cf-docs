...
The SV app OpenAPI specification annotates its endpoints (`x-jvm-package: sv_public`) with an `x-external-audience` extension, which is one of:

- `validators` - endpoints that validator operators need to reach.
